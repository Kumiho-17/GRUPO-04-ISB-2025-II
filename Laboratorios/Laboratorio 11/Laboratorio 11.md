...
5. [Metodología](#5-metodología)  
   - 5.1. [Carga del dataset y selección de la señal SVTA](#51-carga-del-dataset-y-selección-de-la-señal-svta)  
   - 5.2. [Visualización de la señal](#52-visualización-de-la-señal)  
   - 5.3. [Detección del complejo QRS](#53-detección-del-complejo-qrs)  
   - 5.4. [Detección de la onda P](#54-detección-de-la-onda-p)  
   - 5.5. [Detección de la onda T](#55-detección-de-la-onda-t)  
   - 5.6. [Zoom en un ciclo cardíaco](#56-zoom-en-un-ciclo-cardíaco)  
   - 5.7. [Cálculo de intervalos RR y frecuencia cardíaca](#57-cálculo-de-intervalos-rr-y-frecuencia-cardíaca)
...
En este laboratorio se trabaja con una señal real perteneciente a la categoría **SVTA** (Supraventricular Tachycardia), una arritmia caracterizada por actividad eléctrica rápida originada por encima de los ventrículos. La SVTA suele presentar frecuencias cardíacas elevadas, variación en la relación P–QRS y, en muchos casos, ondas P atípicas o superpuestas al QRS, lo que dificulta su identificación visual [3]. El análisis computacional permite reforzar esta delimitación mediante técnicas avanzadas de procesamiento, tales como filtrado, detección automática de picos y algoritmos especializados para ondas P y T.

El objetivo del estudio realizado es detectar y visualizar las ondas **P**, **QRS** y **T** de una señal ECG real mediante el uso de métodos automatizados, integrando herramientas como *neurokit2* y algoritmos establecidos para la identificación de complejos cardíacos. La combinación de estas técnicas facilita la interpretación de un ritmo supraventricular acelerado, permitiendo observar alteraciones típicas como intervalos RR acortados y variabilidad dinámica en la frecuencia cardíaca. De esta manera, el laboratorio conecta los fundamentos fisiológicos del ECG con métodos modernos de análisis digital.

# 2. Fundamentos Teóricos

## 2.1. Electrocardiograma (ECG)

El electrocardiograma registra las variaciones del campo eléctrico cardíaco generadas durante cada ciclo cardíaco. Estas variaciones reflejan la actividad del sistema de conducción: despolarización de aurículas, despolarización de ventrículos y repolarización ventricular. Desde un punto de vista biofísico, el ECG puede interpretarse como una serie de transiciones entre configuraciones eléctricas relativamente estables en el miocardio, cuyo análisis permite inferir la integridad de la conducción eléctrica y la función cardíaca global [1]. Gracias a su accesibilidad y alto valor diagnóstico, el ECG continúa vigente como herramienta clínica estándar incluso frente a metodologías más complejas [2].
...
Cada una de estas ondas posee características distintivas en amplitud, duración y posición temporal dentro del ciclo cardíaco. Su análisis conjunto permite evaluar el origen del ritmo, la relación auriculoventricular y la presencia de anomalías en la conducción o en la repolarización [1], [4]. En situaciones patológicas, estas morfologías pueden alterarse, superponerse o deformarse, dificultando su identificación manual.

## 2.3. Taquicardia Supraventricular (SVTA)

La taquicardia supraventricular (SVTA) agrupa un conjunto de ritmos rápidos cuya actividad eléctrica se origina por encima de los ventrículos, ya sea en el nodo auriculoventricular, las aurículas o vías accesorias. Clínicamente, la SVTA se caracteriza por frecuencias cardíacas elevadas (comúnmente entre 140 y 250 bpm), intervalos RR cortos y, en algunos casos, ondas P ocultas o invertidas dependiendo del mecanismo de reentrada o foco ectópico involucrado [3]. Estas alteraciones pueden generar episodios de conducción irregular, variabilidad en la morfología del QRS o cambios en la relación temporal entre P y QRS, lo que plantea un reto diagnóstico cuando se analizan señales ruidosas o de corta duración.

El uso de algoritmos computacionales para detectar automáticamente los complejos QRS, así como las ondas P y T, resulta especialmente útil en la caracterización de ritmos supraventriculares, permitiendo cuantificar de manera objetiva parámetros como los intervalos RR, la frecuencia cardíaca y la morfología de los ciclos cardíacos.
...
- Cargar, visualizar y describir la señal ECG seleccionada del dataset ECG (1000 fragments).
...
Se utilizó el dataset **ECG signals (1000 fragments)**, disponible en Mendeley Data, el cual contiene registros de señales electrocardiográficas categorizadas en diferentes tipos de ritmos cardíacos. Cada archivo corresponde a un fragmento de señal en formato digital, previamente muestreado y etiquetado.
...
En la Figura 1 se observa el fragmento de señal ECG seleccionado del dataset, clasificado como un episodio de taquicardia supraventricular (SVTA). La señal presenta una morfología característica, con complejos QRS estrechos y de alta amplitud distribuidos a intervalos regulares pero notablemente **acortados**, lo cual es coherente con una frecuencia cardíaca acelerada típica de ritmos supraventriculares.
...
A pesar de estas limitaciones fisiológicas y morfológicas, el algoritmo empleado logra localizar un número consistente de ondas P mediante análisis en banda baja y reconocimiento de patrones pre-QRS. Esto permite resaltar la actividad auricular incluso en situaciones donde la frecuencia elevada y la cercanía temporal entre ondas tienden a ocultar su morfología en el trazado.
...
En el intervalo alrededor de **2.55 segundos** se evidencia un **solapamiento parcial entre la onda P y el inicio del complejo QRS**. Esta superposición es una característica frecuente en episodios de taquicardia supraventricular, donde la elevada frecuencia cardíaca reduce el intervalo PR y puede hacer que la actividad auricular aparezca muy próxima —o incluso fusionada— con la despolarización ventricular.
...
La Figura 6 muestra un acercamiento a un ciclo cardíaco con el fin de analizar en detalle la morfología de las ondas P, QRS y T. Este nivel de ampliación permite distinguir eventos eléctricos que, en la visualización global, pueden aparecer solapados debido a la elevada frecuencia cardíaca característica del episodio de SVTA.

Alrededor de **2.55 segundos** se observa un fenómeno relevante: la **onda T del ciclo previo presenta una amplitud muy reducida** y aparece seguida muy de cerca por la **onda P del siguiente ciclo**. Como consecuencia, ambas ondas pueden parecer superpuestas o fusionadas. Este comportamiento es frecuente en ritmos supraventriculares rápidos, donde el **intervalo RR acortado** reduce la separación temporal entre la repolarización ventricular (T) y la siguiente despolarización auricular (P).

En este segmento se aprecia que:
- La **onda P** mantiene su morfología de baja amplitud y corta duración.  
- El **complejo QRS** sigue siendo estrecho y de alta amplitud, coherente con un origen supraventricular.  
- La **onda T** es visible pero de poca prominencia, lo que facilita la aparente fusión temporal con la onda P.  
...
La Figura 7 muestra la frecuencia cardíaca instantánea obtenida a partir de los intervalos RR detectados en la señal ECG. Los valores iniciales se encuentran notablemente elevados, con picos entre **150 y 162 bpm**, lo cual es característico de la taquicardia supraventricular, cuyo rango típico puede superar los 140 bpm según lo reportado en la literatura clínica [3].
...
Los resultados obtenidos permiten caracterizar de manera adecuada la actividad eléctrica correspondiente a un episodio de taquicardia supraventricular (SVTA). La morfología global de la señal mostró complejos QRS estrechos, intervalos RR marcadamente acortados y ondas P de baja amplitud, patrón que coincide con la descripción fisiológica de los ritmos supraventriculares rápidos [3]. La delineación automática realizada por neurokit2 logró identificar los componentes principales del ciclo cardíaco incluso en condiciones de alta frecuencia, lo cual confirma la utilidad de los métodos algorítmicos para análisis de ECG en contextos complejos [2].

Uno de los hallazgos relevantes se observó en el análisis con zoom, donde la onda **T** de un ciclo presentaba una amplitud reducida y aparecía muy próxima a la onda **P** del ciclo siguiente, generando un aparente solapamiento. Este fenómeno es consistente con la fisiología descrita en los episodios de SVTA, donde la repolarización ventricular puede quedar temporalmente adyacente a la siguiente activación auricular debido a la reducción extrema del intervalo RR [3]. La literatura reporta que la visibilidad de las ondas P y T puede verse alterada en ritmos acelerados, especialmente cuando estas ondas presentan amplitudes pequeñas o se superponen dentro del ciclo eléctrico [1].

El análisis de la frecuencia cardíaca instantánea mostró un inicio del registro con valores entre 150 y 160 bpm, seguido por una disminución progresiva y cierta variabilidad entre ciclos. Estos cambios concuerdan con patrones observados en taquicardias supraventriculares, donde pueden presentarse fluctuaciones en la conducción auriculoventricular o episodios transitorios de compensación fisiológica [3]. La variabilidad observada demuestra que la señal seleccionada representa un episodio dinámico y no un ritmo completamente estable, lo cual aporta información útil para la interpretación clínica del fragmento.

Finalmente, la correcta identificación de las ondas P, QRS y T —incluso bajo condiciones de difícil visualización— confirma que la combinación de técnicas de filtrado, análisis morfológico y algoritmos de detección es adecuada para la delineación del ECG. Esto coincide con revisiones recientes que destacan que, a pesar del surgimiento de técnicas avanzadas, el ECG continúa siendo un método esencial en el análisis clínico y experimental debido a su alta capacidad para identificar alteraciones sutiles en la actividad eléctrica del corazón [2].

# 7. Discusión

Los resultados obtenidos permiten caracterizar con claridad la actividad eléctrica correspondiente a un episodio de taquicardia supraventricular (SVTA). La morfología global de la señal —complejos QRS estrechos, intervalos RR acortados y ondas P de baja amplitud— coincide con lo descrito en la literatura para ritmos supraventriculares rápidos [3]. Este tipo de taquicardia se caracteriza por un mecanismo de activación auricular acelerado y conducción a través del nodo AV, lo que produce frecuencias cardíacas elevadas y un patrón rítmico distintivo. La señal analizada refleja estas características, confirmando su coherencia fisiológica.

En cuanto a la delineación del ECG, el algoritmo empleado logró identificar de manera consistente las ondas P, QRS y T incluso en condiciones de alta frecuencia cardíaca. La detección de picos R es fundamental para el análisis del ciclo cardíaco, y la literatura subraya que los algoritmos modernos presentan un desempeño robusto en la identificación del complejo QRS, incluso en señales con ruido y morfologías complejas [5]. Además, la onda T fue detectada adecuadamente, lo que concuerda con los reportes sobre la importancia del análisis de la repolarización ventricular mediante técnicas automáticas de procesamiento [5].

Un hallazgo relevante se observa en el análisis con zoom del ciclo cardíaco, donde la **onda T del ciclo previo**, de baja amplitud, aparece muy próxima a la **onda P del ciclo siguiente**, generando un aparente solapamiento alrededor de 2.55 s. Este comportamiento es característico de ritmos supraventriculares rápidos, donde la reducción extrema del intervalo RR hace que la repolarización ventricular y la siguiente activación auricular ocurran con muy poca separación temporal [3]. Además, estudios avanzados sobre detección de la onda P han demostrado que esta puede volverse menos visible o confundirse con la onda T en patologías y condiciones de alta frecuencia, dificultando su reconocimiento tanto visual como computacional [4].

La frecuencia cardíaca instantánea mostró valores iniciales entre 150 y 160 bpm, seguidos de una disminución progresiva y oscilaciones marcadas entre ciclos. Esta variabilidad concuerda con lo reportado en episodios de SVTA, donde pueden presentarse fluctuaciones en la relación auriculoventricular y en la estabilidad del ritmo supraventricular [3]. Tales variaciones también evidencian la naturaleza dinámica de la señal, reforzando la importancia del análisis de la variabilidad del ritmo cardíaco como complemento al estudio morfológico.

Finalmente, los resultados obtenidos demuestran que la combinación de técnicas de filtrado, análisis morfológico y algoritmos automáticos sigue siendo fundamental para el estudio del ECG. Revisiones recientes señalan que, pese a la proliferación de técnicas avanzadas, el ECG continúa siendo un método indispensable para la investigación y la práctica clínica debido a su capacidad para identificar patrones eléctricos y distinguir entre diferentes tipos de arritmias [2]. Los hallazgos aquí obtenidos se alinean con esta evidencia, mostrando que el procesamiento computacional del ECG permite comprender con mayor detalle las características fisiológicas de un episodio de SVTA.
